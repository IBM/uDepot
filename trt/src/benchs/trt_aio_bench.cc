...
void *t_io(void *arg__) {
    ...
    ret = AIO::pread(arg->fd, buff, arg->buff_size, chunk*arg->buff_size);
    if (ret < 0) {
...
void *t_main_detached(void *arg__) {
    ...
    while (nops_submitted < nops_total) {
        ...
        if (get_tls_AioState__()->aio_pending_ + nops_batch > AioState::aio_maxio_) {
            T::yield();
            continue;
        }
...
void *t_init(void *arg__)
{
    ...
    T::spawn(AIO::poller_task, nullptr, nullptr, true, TaskType::TASK);
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2) {
        char tempname[] = "/tmp/trt-aio-bench-test-XXXXXX";
        fname = tempname;
